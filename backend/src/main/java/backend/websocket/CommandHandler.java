...
public class CommandHandler {
    ...
    private void handleSendMessage(final SendMessageWebSocketMessage sendMessageWebSocketMessage, final WebSocketConnection connection) {
        ...
        final List<ReceiveUpdatedChatWebSocketMessage> updatedChats = chatUserViews.stream().map(chatUserView -> new ReceiveUpdatedChatWebSocketMessage(chatUserView, chatUserView.getChatBox())).toList();
        ...
        blWebSocketService.sendMessageToChat(sendMessageWebSocketMessage.chatId(), updatedChats, blMessageView);
    }
